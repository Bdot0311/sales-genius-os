  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container mx-auto flex flex-col items-center px-5 text-center sm:px-6">
        ...
        <div
          className={`mb-8 grid w-full max-w-[22rem] grid-cols-3 gap-3 text-center text-[11px] text-white/30 transition-all duration-700 delay-[250ms] sm:mb-10 sm:flex sm:max-w-none sm:items-center sm:justify-center sm:gap-6 sm:text-xs ${visible ? "opacity-100" : "opacity-0"}`}
        >
          {trustPoints.map((point) => (
            <span key={point} className="leading-snug">
              {point}
            </span>
          ))}
        </div>
        ...
        <div
          className={`hidden flex-col items-center gap-3 transition-all duration-700 delay-[320ms] sm:flex ${visible ? "opacity-100" : "opacity-0"}`}
        >
          <p
            className="text-[10px] font-medium uppercase tracking-[0.22em] text-white/20"
          >
            Connects with the tools you already use
          </p>
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            {integrations.map((name) => (
              <span key={name} className="text-xs font-medium tracking-wide text-white/25">
                {name}
              </span>
            ))}
          </div>
