      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-5xl mx-auto">
          {plans.map((plan, index) => (
            ...
            >
              <div className="mb-6">
                <h3 className={`text-xl sm:text-2xl font-bold mb-2 ${plan.highlighted ? 'text-white' : ''}`}>
                  {plan.name}
                </h3>
                <div className={`text-3xl sm:text-4xl font-bold mb-2 ${plan.highlighted ? 'text-white' : ''}`}>
                  {plan.price}
                  {plan.price !== "Custom" && <span className="text-base sm:text-lg font-normal text-muted-foreground">/mo</span>}
                </div>
              ...
              <ul className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
                {plan.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start gap-2">
                    <Check className={`w-4 h-4 sm:w-5 sm:h-5 shrink-0 mt-0.5 ${plan.highlighted ? 'text-white' : 'text-primary'}`} />
