...
serve(async (req) => {
  ...
  try {
    ...
    let plan: 'free' | 'starter' | 'growth' | 'pro' | 'agency' = 'free';
    ...
    if (hasActiveSub) {
      ...
      if (userId) {
        const leadsLimit = (plan as string) === 'free' ? 0 : (plan as string) === 'starter' ? 1000 : (plan as string) === 'growth' ? 2500 : (plan as string) === 'agency' ? 15000 : 5000;
